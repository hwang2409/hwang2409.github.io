...
export default function ContactSoftnessWidget() {
  ...
  return (
    <section className={`project-widget ${styles.demo}`} aria-label="contact softness">
      <InteractiveCanvas draw={draw} resetKey={reset} aria-label="a dropped ball compresses a spring contact; penetration depth appears above" />
      <div className={`project-widget-controls ${styles.controls}`}>
        <label htmlFor={id}>softness: {run.softness.toFixed(2)} · {contactStiffness(run.softness).toFixed(0)} N/m
          <input id={id} type="range" min="0" max="1" step="0.01" value={run.softness} onChange={e => restart(Number(e.target.value))} />
        </label>
        <button type="button" onClick={() => restart(run.softness)}>[drop again]</button>
      </div>
      <p className="project-widget-hint">0 = hard, 1 = soft. unit mass, spring–damper contact, fixed damping ratio 0.18. stiffness is an analogy for compliance, not newt’s solimp mapping.</p>
    </section>
  );
}
